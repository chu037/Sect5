...
CommandText='<%# "SELECT * FROM t_group where t_g_name like ? and (t_g_num like ?) ORDER BY t_g_id ASC" %>'
Debug="true"
><Parameters>
<Parameter  Name="@t_g_name"  Value='<%# "%"+IIf((Request.QueryString("t_g_name") <> Nothing), Request.QueryString("t_g_name"), "%")+"%" %>'  Type="WChar"  />
<Parameter  Name="@t_g_num"  Value='<%# IIf((Request.QueryString("t_g_num") <> Nothing), Request.QueryString("t_g_num"), "%")+"%" %>'  Type="WChar"  /></Parameters>
</MM:DataSet>
...
<title>查詢結果</title><!--參數應照順序 %為萬用字元 like 前後 + "%"-->
<style type="text/css">
<!--
.style2 {
	color: #CC6600;
...
<Columns>
<asp:HyperLinkColumn DataNavigateUrlField="t_g_id" DataNavigateUrlFormatString="s01total_g_result.aspx?t_g_id={0}"
...
<asp:BoundColumn DataField="t_g_num" 
  ...
  <asp:TextBox ID="t_g_name" ReadOnly="true" runat="server" text='<%# comb1(DataSet2.FieldValue("t_g_name", Container)) %>' Visible="false" />  
...
function po_back() { //v3.0
window.opener.document.getElementById("t_g_1_t").value = "<% =t_g_1.text %>";
window.opener.document.getElementById("t_g_2_t").value = "<% =t_g_2.text %>";
window.opener.document.getElementById("t_g_3_t").value = "<% =t_g_3.text %>";
window.opener.document.getElementById("t_g_num_t").value = "<% =t_g_num.text %>";
window.opener.document.getElementById("t_g_id_t").value = "<% =t_g_id.text %>";

window.opener.document.getElementById('Button01').click();
window.opener.document.getElementById('Button01').Enabled="flse";
