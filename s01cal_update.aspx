...
CommandText='<%# "SELECT m_date, m_date_en, m_hours, m_messeng, m_note, m_num, m_time, m_time_en, m_detail FROM s01_calen  WHERE m_num = ?" %>'
Debug="true"
><Parameters>
...
ConnectionString='<%# System.Configuration.ConfigurationSettings.AppSettings("MM_CONNECTION_STRING_s01") %>'
DatabaseType='<%# System.Configuration.ConfigurationSettings.AppSettings("MM_CONNECTION_DATABASETYPE_s01") %>'
CommandText='<%# "SELECT * FROM s01_calen_appen WHERE m_num = ? ORDER BY m_appen_id DESC" %>'
Debug="true"
>
  <Parameters>
...
<script language="VB" runat="server">
 dim url, url_back
  Sub page_load(sender As Object, e As EventArgs)
	...
	if m_numcheck = "" then
	 ...
	 if t_now = 1 then
	  url_now = "s01cal_upfile.aspx?m_num=" & m_numcheck
 ...
 datagrid1.visible = false
  ...
  Sub UpdateData(sender As Object, e As EventArgs) 
    if IsValid Then
	  ...
	  if m_date_en.text <> "" then
	  SQL = "UPDATE calen SET m_time=@m_time, m_time_en=@m_time_en, m_hours=@m_hours, m_messeng=@m_messeng, m_date=@m_date, m_date_en=@m_date_en, m_detail=@m_detail, m_note=@m_note WHERE m_num=@m_num"
	  ...
	  SQL = "UPDATE calen SET m_time=@m_time, m_hours=@m_hours, m_messeng=@m_messeng, m_date=@m_date, m_note=@m_note, m_detail=@m_detail WHERE m_num=@m_num"
      Cmd = New OleDbCommand( SQL, Conn )
 ...
 dim trans_text = "s01cal_upfile.aspx?m_num=" & m_num.text
 if trans_text <> "" then
